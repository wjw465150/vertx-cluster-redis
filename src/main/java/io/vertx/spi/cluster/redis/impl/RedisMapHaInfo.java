...
class RedisMapHaInfo extends RedisMap<String, String> implements NodeAttachListener {
      ...
      removedListeneId = mapAsync.addListener(new EntryRemovedListener<String, String>() {
        ...
        public void onRemoved(EntryEvent<String, String> event) {
          ...
          if (nodeListener != null) {
            log.debug(MessageFormat.format("removed nodeLeft nodeId: {0}, selfNodeId: {1}", nodeId, selfNodeId));
    ...
    if (expiredListenerId == 0) {
      expiredListenerId = mapAsync.addListener(new EntryExpiredListener<String, String>() {
        ...
        public void onExpired(EntryEvent<String, String> event) {
          ...
          if (nodeListener != null) {
            log.debug(MessageFormat.format("expired nodeLeft nodeId: {0}, selfNodeId: {1}", nodeId, selfNodeId));
            nodeListener.nodeLeft(nodeId);
          }
        }
      });
    ...
    if (createdListenerId == 0) {
      createdListenerId = mapAsync.addListener(new EntryCreatedListener<String, String>() {
        ...
        public void onCreated(EntryEvent<String, String> event) {
          String nodeId = event.getKey();
          if (nodeListener != null) {
            log.debug(MessageFormat.format("created nodeAdded nodeId: {0}, selfNodeId: {1}", nodeId, selfNodeId));
    ...
    if (updatedListenerId == 0) {
      updatedListenerId = mapAsync.addListener(new EntryUpdatedListener<String, String>() {
        ...
        public void onUpdated(EntryEvent<String, String> event) {
          String nodeId = event.getKey();
          if (nodeListener != null && !clusterManager.getNodeId().equals(nodeId)) {
            log.debug(MessageFormat.format("updated nodeAdded nodeId: {0}, selfNodeId: {1}", nodeId, selfNodeId));
            nodeListener.nodeAdded(nodeId);
          }
          if (nodeListener != null && clusterManager.getNodeId().equals(nodeId)) {
            ttlAgent.start();
  ...
  public String put(String key, String value) {
    ...
    if (refreshKv.size() != 1) {
      log.warn(MessageFormat
          .format("(refreshKv.size() != 1), key: {0}, value: {1}, refreshKv.size: {2}", key, value, refreshKv.size()));
    }
  ...
  public String remove(Object key) {
    ...
    if (!refreshKv.isEmpty()) {
      log.warn(MessageFormat.format("(!refreshKv.isEmpty()), key: {0}, refreshKv.size: {1}", key, refreshKv.size()));
    }
  ...
  private void refreshAction(long counter) {
    String selfNodeId = clusterManager.getNodeId();
    if (refreshKv.size() > 1) {
      log.warn(MessageFormat.format("(refreshKv.size() > 1), selfNodeId: {0}, refreshKv.size: {1}, counter: {2}",
          selfNodeId,
          refreshKv.size(),
          counter));
    }
    refreshKv.forEach((k, v) -> {
      asyncTTL.refreshTTLIfPresent(k, timeToLiveSeconds, TimeUnit.SECONDS, ar -> {
        if (ar.failed()) {
          log.warn(MessageFormat.format("Refresh TTL failed: selfNodeId: {0}, counter: {1}, k: {2}, v: {3}, error: {4}",
              selfNodeId,
              counter,
              k,
              v,
          ...
          if (counter % 5 == 0) { // Each 5 times, 5=10/2
            asyncTTL.getTTL(k, ar2 -> {
              if (ar2.failed()) {
                log.warn(
                    MessageFormat.format("Get TTL failed: selfNodeId: {0}, counter: {1}, k: {2}, v: {3}, error: {4}",
                        selfNodeId,
                        counter,
                        k,
                        v,
                ...
                if (ttlMillis < checkMillis) {
                  log.debug(MessageFormat.format(
                      "Get TTL (ttlMillis < checkMillis): selfNodeId: {0}, counter: {1}, k: {2}, ttlMillis: {3}, checkMillis: {4}",
                      selfNodeId,
                      counter,
                      k,
                      ttlMillis,
  ...
  private void replaceAction(String selfNodeId, long counter, String k, String v) {
    getMapAsync().fastPutAsync(k, v, timeToLiveSeconds, TimeUnit.SECONDS)
        .whenComplete((newKey, err) -> {
          if (err != null) {
            log.warn(MessageFormat.format("fastPutAsync: selfNodeId: {0}, counter: {1}, k: {2}, v: {3}, error: {4}",
                selfNodeId,
                counter,
                k,
                v,
            ...
            if (newKey) {
              log.debug(MessageFormat.format("fastPutAsync: selfNodeId: {0}, counter: {1}, k: {2}, v: {3}, newKey: {4}",
                  selfNodeId,
                  counter,
                  k,
                  v,
  ...
  private class TTLAgent {
    ...
    private void start() {
      if (isStarted()) {
        log.debug(MessageFormat.format("timeId: {}, selfNodeId: {0}", timeId, clusterManager.getNodeId()));
        return;
      }
    ...
    private void stop() {
      if (!isStarted()) {
        log.debug(MessageFormat.format("timeId: {0}, selfNodeId: {1}", timeId, clusterManager.getNodeId()));
        return;
      }
      long id = timeId;
      timeId = -1;
